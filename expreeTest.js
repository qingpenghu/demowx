// 作用域 数组 字符串操作 定时器 call,apply,bind  Math 浅拷贝深拷贝  Object.defineProperty
...
（2）闭包会在父函数外部，改变父函数内部变量的值。所以，如果你把父函数当作对象（object）使用，把闭包当作它的
     公用方法（Public Method），把内部变量当作它的私有属性（private value），这时一定要小心，不要随便改变父函数内部变量的值。
 * var  counter = 10;
		function add () {
			var  counter = 0;
			return function () {
